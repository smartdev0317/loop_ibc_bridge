...
const Confirm = (): ReactElement => {
  ...
  console.log('request tx result', requestTxResult)
  ...
  return (
    <StyledContainer>
      ...
      {fromBlockChain === BlockChainType.terra && (
        <StyledSection style={{ flexDirection: 'column', paddingBottom: 0 }}>
          <StyledSpaceBetween style={{ marginBottom: 16 }}>
            ...
            <StyledSecD>
              <StyledSecDText2>
                {`${formatBalance(gasFee, feeDenom)} ${ASSET.symbolOfDenom[feeDenom]
                  }`}
              </StyledSecDText2>
            </StyledSecD>
          </StyledSpaceBetween>
      ...
      {bridgeUsed === BridgeType.axelar ||
        ...
        <>
          <StyledSection>
            <StyledSpaceBetween>
              ...
              <StyledSecD>
                <StyledSecDText2>
                  {`${formatBalance(bridgeFee, asset?.terraToken)} ${asset?.symbol
                    }`}
                </StyledSecDText2>
              </StyledSecD>
            </StyledSpaceBetween>
          ...
          <StyledSection>
            <StyledSpaceBetween>
              ...
              <StyledSecD>
                ...
                >
                  {`${formatBalance(amountAfterBridgeFee, asset?.terraToken)} ${asset?.symbol
                    }`}
                </StyledSecDText>
              </StyledSecD>
            </StyledSpaceBetween>
        ...
        <StyledSection>
          <StyledSpaceBetween>
            ...
            <StyledSecD>
              <StyledSecDText>{`${formatBalance(amount)} ${asset?.symbol
                }`}</StyledSecDText>
            </StyledSecD>
          </StyledSpaceBetween>
        </StyledSection>
      ...
      {requestTxResult?.success && requestTxResult?.hash1 === '' && (
        <StyledSection>
      ...
      {requestTxResult?.success && requestTxResult?.hash1 !== '' && (
        <>
          <StyledSection>
            <StyledSecH style={{ color: '#5592f7', opacity: 1 }}>TX1</StyledSecH>
            <StyledSecD>
              <StyledSecDText2>
                {
                  <ExtLink
          ...
          <StyledSection>
            <StyledSecH style={{ color: '#5592f7', opacity: 1 }}>TX2</StyledSecH>
            <StyledSecD>
              <StyledSecDText2>
                {
                  <ExtLink
                    href={getScannerLink({
                      address: requestTxResult.hash1,
                      type: 'tx',
                    })}
                    style={{ padding: 0, display: 'flex', alignItems: 'center' }}
                  >
                    <div>{UTIL.truncate(requestTxResult.hash1, [15, 15])}</div>
                    <BoxArrowUpRight
                      color="#5592f7"
                      style={{ paddingLeft: 3, marginTop: -2 }}
                    />
