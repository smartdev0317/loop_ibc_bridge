...
const useAuth = (): {
  ...
  relayLogin: ({ user }: { user: User }) => Promise<void>
  logout: () => void
  relayLogout: () => void
  getLoginStorage: () => {
  ...
  const [relayBlockChain, setRelayBlockChain] = useRecoilState(
    SendStore.relayBlockChain
  )
  ...
  const relayLogin = async ({ user }: { user: User }): Promise<void> => {
    if (relayBlockChain === BlockChainType.terra) {
      let localNetwork: LocalTerraNetwork | undefined
      let name = ''
      let chainId: string | number = ''
      if (user.walletType === WalletEnum.TerraWalletConnect) {
    ...
    } else if (isIbcNetwork(relayBlockChain)) {
      const network = await user.signer?.getChainId()
      //const isIbcValidNetwork = ibcChainId[relayBlockChain as IbcNetwork] === network
      if (network) {
        setRelayBlockChain(relayBlockChain)
        setKeplrBaseExt({
          chainID: (await user.signer?.getChainId()) || '',
          name: NETWORK.blockChainName[relayBlockChain],
        })
      } else {
        setIsVisibleNotSupportNetworkModal(true)
        network &&
